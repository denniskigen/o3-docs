...
- Les fichiers de configuration partagés doivent généralement exister à la racine du répertoire monorepo. Ceux-ci incluent :
  ...
  - `vitest.config.ts` - Fichier de configuration Vitest (ou `jest.config.ts` dans les modules legacy encore sur Jest).
  - `rspack.config.js` - Fichier de configuration Rspack (ou `webpack.config.js` dans les modules legacy encore sur Webpack).
