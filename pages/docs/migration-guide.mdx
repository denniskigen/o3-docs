...
```sh copy
yarn up openmrs@5 @openmrs/esm-framework@5 && yarn up openmrs@next @openmrs/esm-framework@next
```
...
This means you have a problem with the `dexie` entry in your Jest configuration's `moduleNameMapper` entry. To fix that, ensure your Jest config has a `moduleNameMapper` entry for `dexie` that looks like this:

```js copy
'^dexie$': require.resolve('dexie')
```

### I'm getting a `Module not found: Error: Can’t resolve ‘@swc-node/loader’` error

To fix this, make sure you install `swc-node/loader` as a devDependency in your frontend module:

```sh copy
yarn add -D @swc-node/loader@^1.3.5
```
