...
[App Context](https://github.com/openmrs/openmrs-esm-core/pull/976) is an application-wide context that can be used to store data that is shared across different parts of the application. To leverage it, you need to define a context with a name and initial values, and then access it somewhere in any of its children. The framework provides two hooks to make it easier:
