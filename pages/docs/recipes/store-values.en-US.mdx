...
Provided by `@openmrs/esm-framework` it`s another way to store data in O3. If you are familiar with [React's context](https://react.dev/reference/react/useContext#reference) it works similarly in the same way. All you
need is to create a global context in some higher level component and access it somewhere in any of its children.
The `useDefineAppContext` hook takes in the name of your context and its initial values.
