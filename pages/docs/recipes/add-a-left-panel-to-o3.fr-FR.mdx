...
- Nous importons le composant `LeftNavMenu` du paquetage `@openmrs/esm-framework`. Ce composant rend le panneau de gauche. Nous importons également le hook `useLeftNav` du même paquet. Ce hook est utilisé pour enregistrer et désenregistrer automatiquement le panneau gauche avec le [LeftNav store](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-react-utils/src/useLeftNav.ts). Le magasin LeftNav est un magasin [Zustand](https://github.com/pmndrs/zustand) qui garde la trace de tous les panneaux de gauche qui ont été enregistrés dans l'application.

- Nous appelons le hook `useLeftNav`, qui prend un objet avec deux propriétés:
...
- Le hook `useLeftNav` gère automatiquement le nettoyage lorsque le composant est démonté, vous n'avez donc pas besoin d'appeler manuellement une fonction de désenregistrement.

- Nous rendons le composant `LeftNavMenu` en tant que premier enfant du composant `BrowserRouter` avec la prop `isChildOfHeader` définie à `true`. Cette prop est requise pour que le composant soit rendu; sans elle, le composant retournera un fragment vide. Cela permet de s'assurer que le panneau de gauche est rendu dans toutes les routes de l'application.
...
<Callout emoji="ℹ️" type="info">
  **Note**: Si vous souhaitez utiliser des traductions pour les titres des liens, vous devrez gérer la fonction `t()` dans le composant `LinkExtension` dans `left-panel-link.component.tsx`, car `getSyncLifecycle` est appelé au moment du chargement du module, avant que les hooks React puissent être utilisés. Vous pouvez modifier le composant `LinkExtension` pour utiliser le hook `useTranslation()` afin d'obtenir des chaînes traduites.
</Callout>
