...
- Avoid these [common testing pitfalls](https://kentcdodds.com/blog/common-mistakes-with-react-testing-library):
  - Using the wrong assertion. Install and use [@testing-library/jest-dom](https://github.com/testing-library/jest-dom) to extend Jest's `expect` with assertions for testing React components.
  - Using the wrong query. Use [this resource](https://testing-library.com/docs/guide-which-query) to choose the right query for the element you want to test.
  - Using `query*` variants for _anything_ except checking if an element exists. Only use `query*` variants for asserting that an element does not exist.
  - Using `waitFor` to wait for elements that can be queried with `screen.find*`.
  ...
  - Incorrectly adding `aria-*`, `role`, and other accessibility attributes to elements. Only add them if the application actually uses them.
  - Not using Testing Library's ESLint plugins: [eslint-plugin-testing-library](https://github.com/testing-library/eslint-plugin-testing-library) and [eslint-plugin-jest-dom](https://github.com/testing-library/eslint-plugin-jest-dom).
  - Using the `cleanup` function. RTL takes care of cleaning up after each test.
  - Not using `screen` to query for elements. Use `screen` for querying elements and debugging assertions.
  - Unnecessarily wrapping things in `act`. Follow [this guide](https://kentcdodds.com/blog/fix-the-not-wrapped-in-act-warning) instead to fix `Not wrapped in act(...)` warnings.
  - Not using `render` or `renderHook` to wrap components in tests.
