...
Provided by `@openmrs/esm-framework` it's another way to store data in O3. If you are familiar with [React's context](https://react.dev/reference/react/useContext#reference), it works similarly. Unlike React's Context, App Context works
across React subtrees, and thus can work in extensions and workspaces. All you need is to create a global context in
some higher level component and access it somewhere in any of its children. The `useDefineAppContext` hook takes in
the name of your context and its initial values.
...
the fallback object after the `nullish coalescing` check is for providing a fallback value when the app context is undefined.
