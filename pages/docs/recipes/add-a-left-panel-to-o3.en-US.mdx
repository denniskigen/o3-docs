...
- We're importing the `LeftNavMenu` component from the `@openmrs/esm-framework` package. This component renders the left panel. We're also importing the `useLeftNav` hook from the same package. This hook is used to register and unregister the left panel with the [LeftNav store](https://github.com/openmrs/openmrs-esm-core/blob/main/packages/framework/esm-react-utils/src/useLeftNav.ts) automatically. The LeftNav store is a [Zustand](https://github.com/pmndrs/zustand) store that keeps track of all the left panels that have been registered in the app.

- We're calling the `useLeftNav` hook, which takes an object with two properties:
...
- The `useLeftNav` hook automatically handles cleanup when the component unmounts, so you don't need to manually call an unset function.

- We're rendering the `LeftNavMenu` component as the first child of the `BrowserRouter` component with the `isChildOfHeader` prop set to `true`. This prop is required for the component to render; without it, the component will return an empty fragment. This ensures that the left panel gets rendered in all the routes of the app.
...
<Callout emoji="ℹ️" type="info">
  **Note**: If you want to use translations for the link titles, you'll need to handle the `t()` function within the `LinkExtension` component in `left-panel-link.component.tsx`, as `getSyncLifecycle` is called at module load time, before React hooks can be used. You can modify the `LinkExtension` component to use `useTranslation()` hook to get translated strings.
</Callout>
