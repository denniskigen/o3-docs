...
<Callout emoji="⚠️" type="warning">
  THIS IS NO LONGER THE ACTIVELY MAINTAINED SOURCE OF TRUTH FOR O3 DEV DOCS. This effort has been moved to the OpenMRS Wiki at https://openmrs.atlassian.net/wiki/x/4QABCQ
</Callout>
...
- Thinking about how to set up your forms in O3. Forms in O3 are built using JSON and conform to this [standard schema](https://github.com/openmrs/openmrs-contrib-json-schemas/blob/main/form.schema.json). If your existing forms were built using HTML Form Entry, you'll need to convert them into JSON schemas. Check out the embedded form builder in O3 for an idea of what this format looks like. The schema editor can allow you to build a new form using a dummy schema scaffold. To learn more about forms in O3, read the `Building forms in O3` recipe.
  ...
  <Callout emoji="ℹ️" type="info">
    An OpenMRS module exists for converting HTML Form Entry schemas into JSON schemas compatible with the O3 standard schema at https://github.com/openmrs/hfe-o3-form-schema-converter. Currently, work is ongoing to improve the tool. Go through the repo's README to learn what's possible.
  </Callout>

## Updating metadata

Follow the steps below to update metadata in your O3 instance:
...
<Callout emoji="ℹ️" type="info">
  It's advisable to use a tag that does not match any existing tag on [Docker Hub](https://hub.docker.com/r/openmrs/openmrs-reference-application-3-frontend/tags) (such as `qa` or specific release versions) to avoid conflicts.
</Callout>
