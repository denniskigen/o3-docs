...
To launch a workspace, use the `launchWorkspace` function from `@openmrs/esm-framework`. If the workspace accepts props, they can be passed as `additionalProps` in the second argument. The `workspaceTitle` property is always accepted, and is used to override the workspace's title (which is defined in its registration).
...
`launchWorkspace` follows these rules when opening a workspace:
