    ...
    <BrowserRouter basename={`${window.getOpenmrsSpaBase()}bed-management`}>
      ...
      <main className={styles.container}>
        <Routes>
          ...
          <Route path="/ward/:ward" element={<WardWithBeds />} />
...
- We're adding the `LeftPanel` component at the top of the `<BrowserRouter>` tree, just outside the routes declarations. This means that the `LeftPanel` gets rendered in all the screens that use this page.
