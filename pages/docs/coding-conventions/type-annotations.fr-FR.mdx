...
- Utilisez l'utilitaire `mocked` (`vi.mocked` pour Vitest, `jest.mocked` pour Jest) pour préserver les informations de type lors du mock de fonctions dans les tests. Par exemple :

  Préférez :

  ```tsx
  const mockedShowSnackbar = vi.mocked(showSnackbar); // Toutes les informations de type sont préservées
  // (ou `jest.mocked(showSnackbar)` si le module est encore sur Jest)
  ```

  À :

  ```tsx
  const mockedShowSnackbar = showSnackbar as Mock;
  ```
