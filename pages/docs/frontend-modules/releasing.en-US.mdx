...
<Callout type="info">
  Release notes should match the format of the [O3 changelog](https://o3-docs.openmrs.org/docs/changelog). This format
  includes a title, a description, and a list of changes. The main categories of changes are `Features`, `Bug Fixes`,
  `Breaking Changes`, `Refactors`, `Docs` and `Tests`. Each category should have a list of changes under it. You might
  need to do a bit more work to get the changelog to match this format. It's worth the effort, as it makes it easier for
  the Release Manager to review the changes and prepare release notes for the community.
</Callout>
