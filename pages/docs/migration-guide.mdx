...
1. [Factor out static metadata into a `routes.json` file](/docs/migration-guide#1-factor-out-static-metadata)
2. [Factor out dynamic metadata into a `startupApp` activator function](/docs/migration-guide#2-factor-out-dynamic-metadata)
...
The `$schema` property points to the [routes schema](https://json.openmrs.org/routes.schema.json) file which is a standard JSON schema that enables your IDE to provide autocompletion and validation for the `routes.json` file.
...
### 3. Make the `pages` and `extensions` arrays named exports
