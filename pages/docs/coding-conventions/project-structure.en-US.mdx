...
- Shared configuration files should generally exist at the root of the monorepo directory. These include:
  ...
  - `vitest.config.ts` - Vitest configuration file (or `jest.config.ts` in legacy modules still on Jest).
  - `rspack.config.js` - Rspack configuration file (or `webpack.config.js` in legacy modules still on Webpack).
