...
Dans OpenMRS, les miettes de pain sont implémentées dans le fichier `index.ts` en utilisant les fonctions `registerBreadcrumb` ou `registerBreadcrumbs` exportées par le paquetage `@openmrs/esm-framework`.
...
Voici un exemple de fils d'Ariane avec des ancêtres/sous-pages (Core v5+) :
