...
1. [Factoriser les métadonnées statiques dans un fichier `routes.json`](/docs/migration-guide#factor-out-static-metadata)
2. [Factoriser les métadonnées dynamiques dans une fonction activatrice `startupApp`](/docs/migration-guide#factor-out-dynamic-metadata)
3. [Mettre à jour les dépendances de base](/docs/migration-guide#upgrade-core-dependencies)
4. [Consulter le guide de dépannage](/docs/migration-guide#troubleshooting)
...
Vérifiez que vous avez la dernière version du framework en lançant: ``sh copy yarn up openmrs@next @enmrs/esm-framework@next
