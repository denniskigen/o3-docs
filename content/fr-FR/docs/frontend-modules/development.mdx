...
La plupart du temps, c'est tout ce qu'un développeur doit faire pour faire fonctionner un serveur de développement local. Une exception notable, cependant, est l'exécution de l’application shell. Vous devrez exécuter le shell directement si vous le souhaitez:
...
Pour lancer l’application shell, vous devrez `cd` dans la monorepo `esm-core` et lancer:

```sh
yarn run:shell
```

Cette commande va démarrer un serveur de développement à `http://localhost:8080/openmrs/spa`. Vous pouvez alors naviguer vers l’application shell à `http://localhost:8080/openmrs/spa/shell`. De là, vous pouvez naviguer vers les applications que vous voulez tester.

Parce que ces applications résident dans le répertoire `apps` de la monorepo `esm-core`, les exécuter dans le contexte de l’application shell rend beaucoup plus facile le test des changements. Cependant, une chose à noter lors de l'utilisation de cette approche est que pour que vos changements se propagent sur votre serveur local, vous devrez peut-être reconstruire votre module frontend. Pour ce faire, exécutez:
...
Si vous prenez un ticket et souhaitez aller au bout rapidement, ce flux fonctionne généralement :

1. **Trouver le bon dépôt/module** grâce aux techniques ci‑dessus.
2. **Lancer le module en local** et reproduire le problème ou vérifier le besoin.
3. **Faire le changement** en respectant les [conventions de code](/docs/coding-conventions/introduction).
4. **Valider localement** (contrôle UI + tests pertinents).
5. **Mettre à jour les traductions** si vous ajoutez ou modifiez des chaînes visibles par l’utilisateur.
6. **Ouvrir une PR** avec un résumé clair et des captures si l’UI a changé.

Blocages fréquents à surveiller :

- **Accès au backend / authentification :** Vous aurez besoin d’un backend OpenMRS (dev3 ou local). Si la connexion échoue, vérifiez les identifiants de l’environnement.
- **Overrides d’import map :** Si un changement n’apparaît pas, vérifiez les overrides dans les devtools et faites un hard refresh pour vider le cache.
- **URLs attendues :** L’app shell tourne sur `/openmrs/spa` et votre module local peut être servi sur un autre port.
- **Feature flags :** Si vous ne voyez pas une nouvelle UI, vérifiez si elle est derrière un feature flag dans le menu des feature flags des Implementer Tools.
...
Oui, absolument! Nous avons une suite de tests pour les modules frontaux dans la monorepo. Selon le dépôt, nous utilisons [Jest](https://jestjs.io/) ou [Vitest](https://vitest.dev/), ainsi que [React Testing Library](https://testing-library.com/docs/react-testing-library/intro/) pour les composants React. Vérifiez le `package.json` du module pour connaître le runner configuré. Consultez le [guide des tests unitaires et d'intégration](/docs/frontend-modules/unit-and-integration-testing) et le [guide des tests end-to-end](/docs/frontend-modules/end-to-end-testing) pour en savoir plus. Envisagez également d’ajouter des tests E2E pour vos modifications. La lecture des guides de test ainsi que des tests dans la base de code devrait vous donner une bonne idée de la façon d'écrire des tests.

## Pousser vos modifications vers GitHub

Une fois que vous avez terminé de travailler sur une fonctionnalité ou un correctif, vous devez transférer vos modifications sur GitHub et déposer un PR. Pour ce faire, compilez vos modifications dans un commit. Nous avons des directives pour écrire des messages de validation dans notre [guide de contribution](/docs/frontend-modules/contributing).
...
- `test` - exécute les tests (Jest ou Vitest selon le dépôt).
...
Si vous remarquez que votre application cesse soudainement de fonctionner, cela peut être dû au fait qu'une bibliothèque principale a été modifiée dans l’application shell sur le serveur sur lequel vous développez. Les principales dépendances partagées par tous les modules frontaux d'OpenMRS sont `openmrs` et `@openmrs/esm-framework`. Elles peuvent être mises à jour comme suit:
