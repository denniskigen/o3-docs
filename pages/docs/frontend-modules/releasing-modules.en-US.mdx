...
Once your release commit is merged, the CI workflow's `pre-release` job gets triggered and publishes a pre-release version tagged `next` on npm. This is useful for early testing before you publish `latest`. Under the hood, this job runs the `ci:publish-next` script.
...
This command publishes a new package tagged `latest`. This is the version consumers will get by default when they install your frontend module.

Broadly speaking, when a PR is merged, a `pre-release` job in our primary GitHub Actions CI workflow gets triggered. This job publishes a version of the frontend module to its corresponding npm registry tagged `next`.
...
- `.github/workflows/ci.yml` to see which tags are published on `push` vs `release`.
