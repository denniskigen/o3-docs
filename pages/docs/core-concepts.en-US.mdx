...
Before 2023, frontend modules were loaded synchronously, which meant that the application would not render until all frontend modules had been loaded. This was a known performance bottleneck. In 2023 significant work allowed frontend modules to be loaded *on-demand*, which significantly improved the performance of the application. You can follow the progress [here](https://github.com/openmrs/openmrs-esm-core/pull/671).

## Versions of frontend modules explained

Each frontend module is an NPM package with a name ending in -app. OpenMRS ESMs are released with three different tags: "next", "latest", and a version number.  
 * :red_circle::warning::construction: **"next" = Pre-release, in development.** Newest, cutting edge, still under construction. "next" always refers to the most recent but not-yet-released version of an ESM (e.g., 3.2.1-pre.1067). *Versions labelled "next" are not recommended for production use* as they are considered unstable works-in-progress and have often not undergone integration testing.
 * :yellow_circle: **"latest" = Most Recent Release.** "latest" always refers to the most recent released version of an ESM (e.g., 3.2.0). While you are free to go with the "latest" version of any ESM, you can have more control by specifying the exact version number of each ESM you use. 
 * :green_circle: :white_check_mark: **v X.X.X = A specific version.** A version number always refers to a specific build of each ESM. For example, 3.2.0 or 3.2.1-pre.1067 are both specific versions of the @openmrs/esm-api ESM, though the latter is a pre-release version.
...
Ideally, it is preferable to have one module loading strategy, and work is [in the pipeline](/docs/roadmap) to harmonize these approaches.
