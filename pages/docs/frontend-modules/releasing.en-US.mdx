...
<Callout type="info">
  We're currently working with a bi-weekly release cadence for O3. This means we aim to publish new versions of our
  frontend to our [QA environment](https://test3.openmrs.org/openmrs/spa) every fortnight. QA review and feedback
  typically takes a week, after which we promote the release to our [demo Production
  environment](https://o3.openmrs.org/openmrs/spa). This cadence is subject to change as we get more experience with the
  process.
</Callout>
...
Commit these changes with the title `(chore) Release vx.x.x` where x.x.x is a placeholder for the version number. See an example of [this commit](https://github.com/openmrs/openmrs-esm-patient-chart/pull/1279) that bumps `Patient Chart` to `v5.0.0`.
