...
- [Vitest](https://vitest.dev) (the default across the majority of modules) or [Jest](https://jestjs.io) (in the small number of modules still on the legacy runner), together with [React Testing Library](https://testing-library.com/react), for unit and integration testing. See the [migration guide](/docs/migrate-to-rspack-and-vitest) if you're moving a module from Jest to Vitest.
...
- Avoid these [common testing pitfalls](https://kentcdodds.com/blog/common-mistakes-with-react-testing-library):
  ...
  - Don't use the wrong assertion. Install and use [@testing-library/jest-dom](https://github.com/testing-library/jest-dom) to extend `expect` with DOM-specific matchers for testing React components. The library works with both Vitest (via the `@testing-library/jest-dom/vitest` subpath import) and Jest — the name is historical.
...
- Don't clear mocks in `beforeEach` hooks. Most mocks should be cleared automatically by the testing framework if you have `clearMocks: true` set in your root-level test config:
...
- Don't mock components. Instead, render them with the real implementation. This makes it easier to catch regressions when refactoring components.
  ...
  // (or `jest.mock(...)` if the module is still on Jest)
  ```
