...
To do so, we want to ensure that we have a component that we can use to create links for the left panel:
...
Next, we'll modify our `index.ts` file to add named exports for extensions in the left panel using the `createLeftPanelLink` component from above:
