...
- **5-10x faster core tooling** through Rspack migration for build system
- **~3x faster build times** for applications (sub-1-minute builds)
...
- **Modern ESM architecture** - native ES module support with improved tree-shaking
- **Enhanced tooling** - migration from Jest to Vitest for better ESM compatibility
...
N/B: If you're using `@openmrs/esm-styleguide` as a direct dependency in your frontend module, you'll need to bump it to `next` as well.
...
Bump Carbon and Webpack dependencies to the following versions:
...
If you've pulled the latest changes and the dev server won't start, make sure you've run `yarn` to get the latest dependencies. The new architecture requires updated peer dependencies.
...
- [Billing App](https://github.com/openmrs/openmrs-esm-billing-app/pull/127)
- [Dispensing App](https://github.com/openmrs/openmrs-esm-dispensing-app/pull/214)
- [Fast Data Entry App](https://github.com/openmrs/openmrs-esm-fast-data-entry-app/pull/114)
- [Form Builder](https://github.com/openmrs/openmrs-esm-form-builder/pull/547)
- [Laboratory App](https://github.com/openmrs/openmrs-esm-laboratory-app/pull/188)
- [Stock Management](https://github.com/openmrs/openmrs-esm-stock-management/pull/300)
- [Template App](https://github.com/openmrs/openmrs-esm-template-app/pull/51)
...
yarn turbo build --force # Test sequential builds without cache
```

### Package architecture: Why your imports changed

We reorganized the framework packages to be more modular and efficient.
...
```typescript
// Everything was in one giant esm-api package
import { openmrsFetch, getPatient, getVisits } from "@openmrs/esm-api";
```
...
import { openmrsFetch } from "@openmrs/esm-api"; // Core HTTP/auth
import { getPatient } from "@openmrs/esm-emr-api"; // Medical records
```

**Why we did this:** If your app only needs to make API calls but doesn't work with patients, you don't have to download the patient management code. Smaller bundles for everyone.

#### Framework package cleanup

We also modernized how packages expose their functionality:
...
- **Modern browsers:** Full ES module support for optimal performance
- **Older browsers:** Automatic transpilation maintains compatibility
- **Node.js:** Requires Node 16+ for ES module support
...
- Migrating Patient Chart and Patient Management to Core v6.3.1-pre.2986 (ongoing)
