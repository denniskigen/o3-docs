...
- [Core](https://github.com/openmrs/openmrs-esm-core) - the core packages of the O3 frontend that handle cross-cutting concerns such as loading frontend modules, configuration, internationalizaition, rendering the UI, routing, error handling, styling and so forth.
- [Home](https://github.com/openmrs/openmrs-esm-home) - renders the UI for the landing page after the user logs in.
- [Template](https://github.com/openmrs/openmrs-esm-template-app) - a seed app that is usually most developer's first introduction to O3.
- [Patient chart](https://github.com/openmrs/openmrs-esm-patient-chart) - the patient dashboard, including widgets for concerns such as vitals and biometrics, test results, forms, and allergies. This is where you'll find your run of the mill EMR features.
- [Patient management](https://github.com/openmrs/openmrs-esm-patient-management) - contains frontend modules that handle patient-related concerns such as search, patient lists, registration, service queues, and appointments.
- [Angular form engine](https://github.com/openmrs/openmrs-ngx-formentry) - a form engine built originally by AMPATH. Leverages Angular's powerful support for forms to build a fully-fledged form engine with capabilities such as field validation, injection of custom data sources, conditional rendering, historical expressions, and more.
- [React form engine](https://github.com/openmrs/openmrs-form-engine-lib) (WIP) - originally built by the UCSF-IGHS team, this new form engine is built in React and leverages the powerful formik library to provide a robust solution. Work is ongoing to get this engine to a stable version that has feature parity with the Angular form engine.
- [Form builder](https://github.com/openmrs/openmrs-esm-formbuilder) - form builder tool baked into O3. Enables users to build OpenMRS form schemas interactively using an embedded JSON schema editor or an interactive builder UI. It also provides a render tab that renders your form schema using the React form engine. Users can publish their schemas to their backend server, enabling them to access their forms and collect data in the patient chart via the forms widget.
- [Fast data entry app](https://github.com/openmrs/openmrs-esm-fast-data-entry-app) - built by Mekom solutions, enables retrospective data entry.
- [Dispensing app](https://github.com/openmrs/openmrs-esm-dispensing-app) - built collaboratively by PIH and UCSF-IGHS, the medication dispensing app enables implementers to track medication dispensed within OpenMRS.
- [O3 distro refapp config](https://github.com/openmrs/openmrs-distro-referenceapplication) - holds the build configuration for the O3 [reference application](https://dev3.openmrs.org/openmrs/spa).
- [Cohort builder](https://github.com/openmrs/openmrs-esm-cohortbuilder) - enables the user to perform ad-hoc queries for patients with defined characterists, and combines multiple queries into more complex ones.
- [Module management](https://github.com/openmrs/openmrs-esm-module-management-app) - a learning application based off of the legacy module management interfaces. Code in this repo is annotated with comments and can serve as a good starting point for new developers.
- [Frontend RFC](https://github.com/openmrs/openmrs-rfc-frontend) - where architectural decisions affecting technologies and processes that apply to all new O3 distributions are first presented, argued, and finally agreed upon via consensus.
...
Contains the seed starter app. The template app will be most developer's first foray into O3. Most components in the template app are annotated with useful comments that serve as a guide for developers working on the application. In the past, it's also served as a seed app from which developers can spawn new frontend modules.
...
This monorepo contains frontend modules related to registering and editing patients, searching for existing patients, creating and managing patient lists, managing patient queues in an outpatient setting and creating, editing and managing patient appointments. The modules within this repository include:

- [Active visits](https://github.com/openmrs/openmrs-esm-patient-management/tree/main/packages/esm-active-visits-app)
...
Leverages the Reporting Compatibility module to enable users to perform ad-hoc queries for patients with defined characteristics, and combine multiple queries into more complex ones.

### Module management

The module management app is meant to serve as a learning resource for our coding conventions and best practices when building a microfrontend.
