...
Un React Hook est fourni pour masquer l'asynchronicité du chargement de la configuration. Le `moduleName` fourni dans le fichier racine `index.ts` de votre application (aux fonctions `getSyncLifecycle` ou `getAsyncLifecycle`) est utilisé pour rechercher la configuration ailleurs dans l'application.
